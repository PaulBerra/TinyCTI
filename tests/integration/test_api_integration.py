...
class TestAPIIntegration:
    ...
    def api_integration_config(self, temp_directory):
        ...
        return {
            ...
            "logging": {
                "level": "INFO",
                "file": str(temp_directory / "api_test.log")
            },
            "api": {
                "host": "127.0.0.1",
                "port": 5000,
    ...
    def test_api_data_export_integration(self, api_client):
        ...
        response = client.get('/api/export/json/ipv4?bucket=live', headers=headers)
        assert response.status_code == 200
        data = json.loads(response.data)
        assert "data" in data
        
        # Test export CSV domains
        response = client.get('/api/export/csv/domain?bucket=live', headers=headers)
        assert response.status_code == 200
    ...
    def test_api_feed_management_integration(self, api_client):
        ...
        response = client.post('/api/feeds/api_test_feed/schedule', 
    ...
    def test_api_data_consistency(self, api_client):
        ...
        from tinycti import IOC, IOCType, RetentionBucket
        test_iocs = [
            IOC("consistency.test", IOCType.DOMAIN, "test", RetentionBucket.LIVE),
        ...
        # Export JSON
        response = client.get('/api/export/json/domain', headers=headers)
        assert response.status_code == 200
        json_data = json.loads(response.data)
        
        # Export CSV
        response = client.get('/api/export/csv/domain', headers=headers)
        assert response.status_code == 200
        csv_data = response.data.decode('utf-8')
        
        # Export text
        response = client.get('/api/export/text/domain', headers=headers)
        assert response.status_code == 200
        text_data = response.data.decode('utf-8')
        
        # Vérifie que les données sont cohérentes entre les formats
        if "data" in json_data:
            domain_in_json = any("consistency.test" in str(item) for item in json_data["data"])
            domain_in_csv = "consistency.test" in csv_data
            domain_in_text = "consistency.test" in text_data
            
            # Au moins un format devrait contenir les données
            assert domain_in_json or domain_in_csv or domain_in_text
    ...
    def test_api_pagination_large_datasets(self, api_client):
        ...
        large_iocs = []
        for i in range(100):
            ioc = IOC(f"test{i}.example.com", IOCType.DOMAIN, "test", RetentionBucket.LIVE)
        ...
        for limit in [10, 50, 100]:
            response = client.get(f'/api/export/json/domain?limit={limit}', headers=headers)
            assert response.status_code == 200
