...
from tinycti import LoggingConfigurator, ErrorHandler, CircuitBreaker
...
class TestLoggingConfigurator:
    ...
    def test_setup_main_logger_configuration(self, mock_rotating_handler, 
                                           mock_stream_handler, mock_root, 
                                           test_config_minimal):
        ...
        mock_root.handlers = [existing_handler1, existing_handler2]
        
        # Mock des nouveaux handlers
        ...
        # Vérifie que les anciens handlers ont été supprimés
        mock_root.removeHandler.assert_any_call(existing_handler1)
        mock_root.removeHandler.assert_any_call(existing_handler2)
        
        # Vérifie que les nouveaux handlers ont été créés
        mock_rotating_handler.assert_called_once()
        mock_stream_handler.assert_called_once()
        
        # Vérifie que les handlers ont été configurés
        mock_file_handler.setFormatter.assert_called_once()
        mock_console_handler.setFormatter.assert_called_once()
        
        # Vérifie que les handlers ont été ajoutés
        mock_root.addHandler.assert_any_call(mock_file_handler)
        mock_root.addHandler.assert_any_call(mock_console_handler)
