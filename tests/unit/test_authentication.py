...
class TestAuthenticationSystem:
    ...
    def test_jwt_token_verification_expired(self, flask_app):
        ...
        with flask_app.app_context():
            ...
            expired_payload = {
                ...
                - timedelta(hours=1),  # Expiré depuis 1 heure
            }
            expired_token = jwt.encode(
                expired_payload, api.app.secret_key, algorithm="HS256"
            )
            ...
            with flask_app.app_context():
                ...
                with patch.object(api, "_require_auth") as mock_require_auth:
                    mock_require_auth.return_value = (
                        lambda f: f
                    )  # Simule l'authentification réussie
    ...
    def test_api_password_authentication(self, flask_app):
        ...
        with flask_app.app_context():
            mock_tinycti = Mock()
            mock_tinycti.config = {
                "api": {"auth": {"enabled": True, "password": "secret_api_key"}}
            }
            api = TinyCTIAPI(mock_tinycti)

            # Simule une requête avec le bon mot de passe API
            with flask_app.test_request_context(
                headers={"X-API-Password": "secret_api_key"}
            ):
            ...
            mock_tinycti.config = {
                "authentication": {
                    "users": {
                        "testuser": {
                            "password_hash": bcrypt.hashpw(
                                "validpass".encode("utf-8"), bcrypt.gensalt()
                            ).decode("utf-8"),
                            "role": "admin",
                        }
                    }
