...
from tinycti import TinyCTIAPI, RetentionBucket, IOCType
...
class TestTinyCTIAPI:
    ...
    def test_export_endpoint_json(self, flask_client):
        ...
        response = flask_client.get('/api/export/json/ipv4?bucket=live&limit=100')
    ...
    def test_export_endpoint_csv(self, flask_client):
        ...
        response = flask_client.get('/api/export/csv/domain?bucket=chaud&limit=50')
    ...
    def test_export_endpoint_text(self, flask_client):
        ...
        response = flask_client.get('/api/export/text/url?bucket=live')
    ...
    def test_feed_schedule_update_endpoint(self, flask_client):
        ...
        response = flask_client.post('/api/feeds/test_feed/schedule',
...
class TestAPIAuthentication:
    ...
    @patch('flask.request')
    def test_bearer_token_authentication(self, mock_request, flask_app):
        """Test l'authentification par token Bearer"""
        with flask_app.app_context():
            mock_tinycti = Mock()
            mock_tinycti.config = {"api": {"auth": {"enabled": True}}}
            
            api = TinyCTIAPI(mock_tinycti)
            
            # Mock request avec header Authorization
            mock_request.headers = {'Authorization': 'Bearer valid_token'}
            
            with patch.object(api, '_verify_token', return_value=True):
                # Test que l'authentification fonctionne
                pass

    @patch('flask.request')
    def test_api_password_authentication(self, mock_request, flask_app):
        ...
        with flask_app.app_context():
            ...
            # Mock request avec header X-API-Password
            mock_request.headers = {'X-API-Password': 'secret_key'}
...
class TestAPIPerformance:
    ...
    def test_large_dataset_export(self, flask_client):
        ...
        response = flask_client.get('/api/export/json/ipv4?limit=10000')
