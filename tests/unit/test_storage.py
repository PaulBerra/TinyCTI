...
from tinycti import IOC, IOCClassifier, IOCStorage, IOCType, RetentionBucket
...
class TestIOCStorage:
    ...
    def test_store_iocs_with_promotion(self, temp_storage):
        ...
        ioc_chaud = IOC(
            "192.168.1.1", IOCType.IPV4, "source1", retention=RetentionBucket.CHAUD
        )
        temp_storage.store_iocs([ioc_chaud])

        # Même IOC mais dans bucket live (plus prioritaire)
        ioc_live = IOC(
            "192.168.1.1", IOCType.IPV4, "source2", retention=RetentionBucket.LIVE
        )
    ...
    def test_deduplicate_iocs_new(self, temp_storage, sample_iocs):
        ...
        assert len(result["new"]) == len(sample_iocs)
    ...
    def test_deduplicate_iocs_existing(self, temp_storage, sample_iocs):
        ...
        assert len(result["updated"]) == len(sample_iocs)
    ...
    def test_write_iocs_file_size_limit(self, temp_storage):
        ...
        for i in range(10):
            ioc = IOC(
                f"192.168.1.{i}", IOCType.IPV4, "test", retention=RetentionBucket.LIVE
            )
    ...
    def test_database_connection_error_handling(self, temp_directory):
        ...
        with patch("sqlite3.connect", side_effect=sqlite3.Error("DB Error")):
            ioc = IOC(
                "192.168.1.1", IOCType.IPV4, "test", retention=RetentionBucket.LIVE
            )
    ...
    def test_storage_with_malformed_ioc(self, temp_storage):
        ...
        # Test que la validation IOC fonctionne correctement

        # IOC avec valeur vide - devrait lever ValueError
        with pytest.raises(ValueError, match="IOC value must be a non-empty string"):
            IOC("", IOCType.IPV4, "test", retention=RetentionBucket.LIVE)

        # IOC avec seulement des espaces - devrait lever ValueError
        with pytest.raises(ValueError, match="IOC value must be a non-empty string"):
            IOC("   ", IOCType.DOMAIN, "test", retention=RetentionBucket.LIVE)

        # IOC avec source vide - devrait lever ValueError
        with pytest.raises(ValueError, match="IOC source must be a non-empty string"):
            IOC("192.168.1.1", IOCType.IPV4, "", retention=RetentionBucket.LIVE)

        # Test avec IOCs valides pour vérifier que le stockage fonctionne toujours
        valid_iocs = [
            IOC(
                "192.168.1.1",
                IOCType.IPV4,
        ...
        stats = temp_storage.store_iocs(valid_iocs)
        assert stats["total"] == len(valid_iocs)

    def test_bucket_priority_enforcement(self, temp_storage):
        """Test l'application des priorités de bucket"""
        # IOC dans bucket froid
        ioc_froid = IOC(
            "test.com", IOCType.DOMAIN, "source1", retention=RetentionBucket.FROID
        )
        temp_storage.store_iocs([ioc_froid])

        # Même IOC dans bucket live (plus prioritaire)
        ioc_live = IOC(
            "test.com", IOCType.DOMAIN, "source2", retention=RetentionBucket.LIVE
        )
        stats = temp_storage.store_iocs([ioc_live])
...
class TestIOCClassifier:
    ...
    def test_classify_ipv4(self):
        ...
        # IPs publiques (valides pour IOCs) - utilise des IPs non whitelistées  
        public_ipv4 = ["185.199.108.153", "140.82.112.3", "151.101.193.140"]
        ...
        private_ipv4 = ["192.168.1.1", "10.0.0.1", "172.16.1.1", "127.0.0.1"]
